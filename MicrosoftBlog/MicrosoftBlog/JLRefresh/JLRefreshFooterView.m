...
#import "JLRefreshFooterView.h"
#import "JLRefreshConst.h"

@interface JLRefreshFooterView()
//{
//    BOOL _withoutIdle;
//}
{
...
@implementation JLRefreshFooterView

+ (instancetype)footer
{
    return [[JLRefreshFooterView alloc] init];
}
...
{
    // 1.移除以前的监听器
    [_scrollView removeObserver:self forKeyPath:JLRefreshContentSize context:nil];
    // 2.监听contentSize
    [scrollView addObserver:self forKeyPath:JLRefreshContentSize options:NSKeyValueObservingOptionNew context:nil];
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    ...
    if ([JLRefreshContentSize isEqualToString:keyPath]) {
        [self adjustFrame];
    }
}
...
- (void)adjustFrame
{
    ...
    // 设置边框
    self.frame = CGRectMake(0, y, _scrollView.frame.size.width, JLRefreshViewHeight);
}
...
- (void)setState:(JLRefreshState)state
{
	...
	switch (state)
    {
        ...
        {
            _statusLabel.text = JLRefreshFooterReleaseToRefresh;
		...
		case JLRefreshStateNormal:
        {
            _statusLabel.text = JLRefreshFooterPullToRefresh;

            // 刚刷新完毕
            CGFloat animDuration = JLRefreshAnimationDuration;
            CGFloat deltaH = [self contentBreakView];
            CGPoint tempOffset;
            
            int currentCount = [self totalDataCountInScrollView];
            if (JLRefreshStateRefreshing == oldState && deltaH > 0 && currentCount != _lastRefreshCount) {
        ...
        case JLRefreshStateRefreshing:
        {
            ...
            _statusLabel.text = JLRefreshFooterRefreshing;
            _arrowImage.transform = CGAffineTransformMakeRotation(M_PI);
            [UIView animateWithDuration:JLRefreshAnimationDuration animations:^{
                UIEdgeInsets inset = _scrollView.contentInset;
                CGFloat bottom = JLRefreshViewHeight + _scrollViewInitInset.bottom;
                CGFloat deltaH = [self contentBreakView];
...
- (int)viewType
{
    return JLRefreshViewTypeFooter;
}

- (void)free
{
    [super free];
    [_scrollView removeObserver:self forKeyPath:JLRefreshContentSize];
}
@end